...
def process_log_line(line: str, args: argparse.Namespace) -> None:
    ...
    log_result = (
        Result.from_either(json.loads(line))
        if line.strip()
        else Failure("Empty line")
    )
    ...
    if (
        args.level
        and level.upper() != args.level.upper()
        or args.task
        and extra.get("task") != args.task
        or args.event
        and extra.get("event") != args.event
    ):
        return
    ...
    if args.pretty:
        ...
        print(
            f"{get_color(level)}[{level:8}] {time} | "
            f"{extra.get('task', ''):<16} | {msg}{LogLevel.RESET.value}"
        )
